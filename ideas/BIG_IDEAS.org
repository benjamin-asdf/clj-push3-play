...
Cutting edges of evolutionary computing / algorithmic structure of natural selection as far as I currently conceptualize them.

* MMM(U) Evolution: Metaevolution, Multiimensionola Selection, Modularity and Unconvential Information Flows
...
- Doesn't explain all aspects of /beauty in nature/? Dawkins would agrees with the sentiment.
- I suspect that theoretical evolutionary biologists have a richer set of algorithms in mind when they think of natural selection,
  but that the algorithms are not formulated yet.
...
#+begin_src
  ...
  natural-selction(population,threshold):
...
- Because of metaevoution: =Mut= can use metaevolutionary heuristics as an apect of play. A gene could good when it's source code looks good.
...
- fitness function in biology is still /inclusive genetic fitness/, but what are the fitness contexts? an open ended problem because of complex =Mut=.
...
- Claim: Biologists already have a richer alg in mind for natural selection. Just the translation into computer algorithms isn't done yet.
